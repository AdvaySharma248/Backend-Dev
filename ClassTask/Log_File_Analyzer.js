...
// Q6: Piping connects output of one stream to input of another. Example: fs.createReadStream('input.txt').pipe(fs.createWriteStream('output.txt'))
const fs = require('fs');
...
if (logFilePath == undefined || logFilePath == null || logFilePath == '') {
    console.error('Usage: node Log_File_Analyzer.js <log_file_path> [report_file_path]');
    process.exit(1);
}
